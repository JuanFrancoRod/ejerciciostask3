...
botonSwitch.addEventListener("click", () => {
  ...
  let notasFiltradasPorTexto = filtrarPorTexto(
    notasFiltradasPorEstado,
    $buscador.value
  );
  ...
  return `<div class="card m-3 col-10 col-md-3">
                <div class="card-body">
                    <input onClick="marcarRealizada(${
                      nota.id
                    })" type="checkbox" ${nota.realizada ? "checked" : ""}>
                    <h5 class="card-title">${nota.titulo}</h5>
                    <p class="card-text">${nota.texto}</p>
                    <button type="button" class="btn btn-primary" onClick="borrarNota(${
                      nota.id
                    })">Borrar nota</button>
                </div>
            </div>`;
...
function filtrarPorTexto(array, texto) {
  ...
  return array.filter(
    (nota) =>
      nota.titulo.toLowerCase().includes(textoLower) ||
      nota.texto.toLowerCase().includes(textoLower)
  );
}
